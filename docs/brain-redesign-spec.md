...
项目级 vault (`~/.abrain/projects/<id>/vault/`) 由 `resolveActiveProject(cwd, { abrainHome })` 在 `activate()` 中按 ADR 0017 strict binding 做 snapshot；session 中普通 `cd` 不变更 vault 可见性，只有显式 `/abrain bind` / `/abrain status` 会刷新当前进程的 binding snapshot。
