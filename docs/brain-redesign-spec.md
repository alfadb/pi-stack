...
> **状态**：v1.1 — incorporate Round 3 复核 P0 修订（2026-05-09）。Baseline v1.0 accepted alongside [ADR 0014](adr/0014-abrain-as-personal-brain.md)。
>
> **v1.1 变更点**（三家复核 PASS after fixing P0）：
...
**Stage 3 — ambiguous 处理**：如果两阶段后 sediment 仍无法唯一决定（多个 aboutness 特征同时命中），默认写入到 `projects/<active>/observations/staging/needs-review.md` （per-day append）。**不直接写 identity/habits**——这里需要不变量 #1 的 trust budget，漂移代价高。
...
| Surface | 范围 | Boost |
|---|---|---|
| **active project** | `projects/<active>/*` + `projects/<active>/vault/_meta.md` | **2.0×** |
| **about-me + world** | `identity/` `habits/` `skills/` `workflows/` `knowledge/` `vault/_meta.md` | **1.0×**（baseline） |
| **other projects** | `projects/<other-id>/*`（**不含**他们的 vault） | **0.3×** |
...
- **自动解锁仅是 preferred path**（不是不变量）。平台能力差异如下，详细矩阵见 [vault-bootstrap.md §1](migration/vault-bootstrap.md#1-平台支持矩阵)：
  - **macOS**：Keychain Services（`security` CLI）——登录 session 内自动可用
  - **Linux desktop**（GNOME / KDE）：Secret Service / `libsecret`——桌面 session 内自动可用
  - **Linux headless / server**：`pass`（gpg-encrypted file）或 fallback `~/.abrain/.vault-master.age`（用户 GPG identity 加密）——需主动 unlock
  - **WSL**：桥接 Windows Credential Manager 或 Linux headless 路径
  - **CI / 容器**：不支持 vault，bash 注入拒绝、`vault_release` 不可用
- **取不到 master key 时 fail-closed**（不转明文备胎）：vault 元数据可读，value 保持加密，bash 注入拒绝 + 报错提示 `pi vault unlock`。全进程生命周期 TUI footer 持续显示 `vault: locked`
...
> **为什么需要这个节**：Round 3 复核 Opus P0-2 指出——`/secret github-token ghp_xxx` 后用户立刻在 bash 里 `gh repo list` 期望 `$VAULT_github_token` 可用。若 vault 写入走 sediment 的 agent_end 异步 pipeline（典型延迟 5-30s），happy path 会失败。

**决策**：`/secret` / `vault import-env` / `vault forget` 由 sediment **同步处理**——TUI 命令 → IPC 调 sediment 进程 → 同进程加密落盘 → atomic rename → 返回。**不走 agent_end 异步队列**。

**不变量 #1 修订**（同步 ADR 0014）：主 session 不写**记忆类条目**（identity / habits / skills / workflows / projects / knowledge）；vault 写入是**用户物理键入触发的同步管道**，由 sediment 单独执行。主 session LLM 仍然不能发起 vault 写入（调 sediment 的 IPC 需要用户键入触发，不是 tool call）。

**事务语义**：
- 同步返回前加密文件已 atomic rename 到最终路径
- `_meta.md` 同步更新（先写 `_meta.md.tmp` 再 rename）
- 失败场景（keychain locked / 磁盘满 / age 库调用错）TUI 立刻报错，不进入 partial state

**独立 audit log**：Lane V 走 `~/.abrain/.state/vault-events.jsonl`（不与 sediment-events.jsonl 混）——避免与 agent_end 异步事件联屏。详见 ADR 0014 §审计扩展。
...
bash wrapper 按需 lazy-decrypt（仅 expand 该次执行需要的 keys）、注入子 shell env、子进程退出后明文仅存于子 shell。
...
- pi 先收集完整 stdout 到 buffer（默认 10s timeout / 1MB 上限，哪个先到）
- redact 整个 buffer 后返回
- 超时 / 超限 丢弃剩余并记录 truncation notice
...
| **同一 project 的 entry 写入（同 slug）** | 发生概率极低（两个 pi 同一 agent_end 窗口内 sediment 同时 extract 同一洞察）。处理：**短期接受最后写入赢**（markdown SOT 不丢数据，git history 看到一次覆盖跳变，可读但不美观）。**中期**：sediment 写前走语义 dedupe（复用 [memory-architecture.md §8.2](memory-architecture.md) 的 dedupe gate，需确认在 abrain 新路径下仍有效） |
