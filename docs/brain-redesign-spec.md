...
`/secret set/list/forget` 当前 scope 解析（legacy P1 已 ship；B4.5 后增加 strict binding 约束）：
...
- `--project=<id>` → 显式指定其他 project（legacy 支持；B4.5 后仍可作为人工 override，但必须满足该 project 的本机授权策略，或收紧为仅 active project，实施时另行拍板）
