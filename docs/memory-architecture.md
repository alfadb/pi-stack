...
| smell | `staging/` | `staging/` | smell 是 `staging/` 中任意 kind + `confidence < 5` 的事实状态 |
...
- Timeline 内禁止：heading（`#`/`##`/`###`）、code fence（`` ``` ``）、HTML table、嵌套 list
...
memory_search(query):
  ...
  3. Facade 合并两 store 结果：
     ...
     c. 排序：textScore × projectBoost × log(1+citations) × max(0.1, confidence/10)
...
**Transaction 边界**：
- 核心事务 = sanitize → classify → dedupe → lint → lock → write md → git commit
- 任何核心步骤失败 → 回滚：删除 tmp markdown，不提交 git
- **git commit 是事务的完成标志**——commit 后 canonical 数据已安全
- graph.json 索引更新是 **best-effort**：成功则更新，失败则标记 stale（下次查询时 lazy rebuild），不回滚 git
- 多条目的 sediment run：每条目独立事务（各自 commit），互不连锁失败
...
| T9 `no-table-in-timeline` | Timeline 区域内不能出现 `\|` table 行 | ERROR |
...
CI gate：`dead_links > 0` → fail（不允许死链）。其余指标只 warn，不阻塞。
...
- [ ] `_index.md` 自动生成（格式：标题列表 + kind 统计 + 最近更新 top-10；sediment 写入后重建）
...
- [ ] Promotion gates 1-5 实现（LLM judge 模式）
...
### B.6 最终评审（第七轮 T0，2026-05-07）

参与者：Claude Opus 4 / GPT-5.5 / DeepSeek V4 Pro。三模型一致 **CONDITIONAL** → 修订后 **PASS**。

核心收敛：Facade 泄漏（LLM-facing schema 不暴露 scope/backend/source_path）；Phase 1 dedupe 算法明确（trigram Jaccard，无语义模型依赖）；Phase 1 范围缩小为 project-only 可验证子集；kind→目录映射表；sanitize 提前到 dedupe 前；事务边界明确（index=best-effort）；Timeline lint 从 7→10 条；dead_link_rate CI gate 公式修正；conf 采用 floor = 0.1 替代归零。
