# Pi 知识管理架构设计（ARCHIVED v7 original）

> ⚠️ ARCHIVED：本文是 2026-05 v7 monolith 原文，包含已过时的 `.pensieve/` current-write、gbrain 前史、grep/BM25 fallback、旧 phase checklist 等内容。当前规范见 [../memory-architecture.md](../memory-architecture.md)、[../architecture/memory.md](../architecture/memory.md)、[../architecture/sediment.md](../architecture/sediment.md)。
...
6. 上述工具暴露给 LLM 使用；世界级与项目级统一走 markdown 文档存储（原『world 用 gbrain』选项随 gbrain 于 v7 整体退场同期废弃，详本文顶部 ADR overlay 与 ADR 0002 supersede 标注）
...
> ⚠️ **B5 cutover (2026-05-13)：** 下图以 `<project>/.pensieve/` 为根绘制，反映 v7 原始设计。实际物理路径已迁移：
> - 项目级写入目标：`~/.abrain/projects/<id>/`（sediment writer 全切 abrain；`.pensieve/` 仅作 legacy 只读迁移源）
> - Locks / audit：`~/.abrain/.state/sediment/locks/` + `~/.abrain/.state/sediment/audit.jsonl`
> - 逻辑结构（maxims/ / decisions/ / knowledge/ / staging/ / archive/）在 abrain project dir 下保持不变
>
> 权威物理拓扑见 [directory-layout.md](../directory-layout.md) + [brain-redesign-spec.md](../brain-redesign-spec.md)。
...
/memory migrate [--dry-run|--go] [--report]  # B4.5 后要求先 /abrain bind --project=<id>；不再接受 --project
...
# abrain / vault / secret（extensions/abrain/index.ts）
/abrain bind [--project=<id>]                     # B4.5：写 .abrain-project.json + registry + local-map（已实施）
/abrain status                                    # B4.5：展示 strict binding 状态（已实施）
/vault status
...
lint: 内存中校验 frontmatter schema + timeline 格式
  ↓
acquire lock: 获取 abrain 全局 file lock（`~/.abrain/.state/sediment/locks/sediment.lock`）
  ↓
...
- **maxims** (2): `maxims/never-close-main-pane.md`, `maxims/prefer-at-file-for-long-prompts.md`
- **decisions** (3): `decisions/use-pnpm.md`, ...
- **knowledge** (5): [...]
- **staging** (2): [...]

## Recently Updated

- 2026-05-08 | `maxims/never-close-main-pane.md` | confidence:7
- 2026-05-07 | `knowledge/avoid-long-argv-prompts.md` | confidence:3
- ...

## Orphans

- `staging/orphan-entry.md` — no incoming links
```
