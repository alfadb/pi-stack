# Brain Redesign Spec（ARCHIVED v1.5 original）

> ⚠️ ARCHIVED：本文是 v1.5 原文，保留历史设计推演；其中部分命令/路径/状态已被 B5 cutover 与 current docs 取代。当前规范见 [../brain-redesign-spec.md](../brain-redesign-spec.md)、[../architecture/abrain.md](../architecture/abrain.md)、[../architecture/vault.md](../architecture/vault.md)。
...
│   ├── pi-astack/
│   │   ├── _project.json               ← B4.5 registry：project_id + timestamps（tracked）
│   │   ├── decisions/
│   │   ├── knowledge/
│   │   ├── maxims/
...
├── .state/projects/local-map.json      ← B4.5 本机 path 授权缓存（local-only，gitignored，支持多路径）
└── .gitignore                          ← 忽略 runtime state、lock、tmp；不忽略 encrypted vault artifacts

运行模型：
  N 个独立 pi 进程同时跑，互不通信
  每个进程在 boot-time 读取项目仓 `.abrain-project.json` + abrain registry + local-map，三者匹配才得到 active project
...
| `projects/<id>/_project.json` | abrain project registry | `/abrain bind` | ✅ | permanent |
| `.state/projects/local-map.json` | 本机 path 授权（project_id → paths[]） | `/abrain bind` / resolver 更新 last_seen | ❌ | local runtime |
...
#### B4.5 project registry + local map

ADR 0017 取代旧 `_bindings.md` 方案：

```text
<project>/.abrain-project.json                    # portable identity claim（project repo，可选择 commit）
~/.abrain/projects/<project-id>/_project.json     # abrain tracked registry
~/.abrain/.state/projects/local-map.json          # local-only path authorization map（支持多个 paths[]）
```
...
## 5. Project Identity — strict binding（ADR 0017 / B4.5）

### 5.1 身份来源

**project id 是唯一身份**。cwd 与 git remote 都不是身份：cwd 会移动，remote 会 rename / fork / mirror / SSH↔HTTPS 切换。B4.5 不再使用 `_bindings.md`、git remote 或 cwd prefix 自动解析 active project。

三层数据共同决定 active project：

1. `<project>/.abrain-project.json` — project repo 内的 portable identity claim：

   ```json
   { "schema_version": 1, "project_id": "opencode-global" }
   ```

2. `~/.abrain/projects/<project_id>/_project.json` — abrain tracked registry：

   ```json
   { "schema_version": 1, "project_id": "opencode-global", "created_at": "...", "updated_at": "..." }
   ```

3. `~/.abrain/.state/projects/local-map.json` — local-only path authorization map：
   ...
   ```json
   {
     ...
     "projects": {
       "opencode-global": {
         "paths": [
           { "path": "/home/worker/.config/opencode", "first_seen": "...", "last_seen": "...", "confirmed_at": "..." }
         ]
       }
     }
   }
...
`.abrain-project.json` 只是 identity claim，不能单独授权 project vault 或 sediment write；当前 path 必须也在 local-map 中由用户确认。

### 5.2 首次绑定与新路径确认

新项目或旧项目第一次进入 strict mode：
...
如果项目已含 `.abrain-project.json`（例如新 clone 或移动后的路径），用户运行：

```text
/abrain bind
```

命令读取 manifest 的 project_id，确保 registry 存在，并把当前 path 加入 local-map 的 `paths[]` 或更新 `last_seen`。这一步是本机授权确认。

不提供 `--local-only`。如果用户不希望 `.abrain-project.json` 随 repo 传播，可以选择不 commit 或自行复制/传输；工具不增加分支语义。
...
拒绝范围：`/memory migrate --go`、sediment project writes、project vault scope（`vault_release(scope='project')`、`/secret` 默认 project scope、`$PVAULT_*` / `$VAULT_*` project lookup）。读工具可继续运行。
...
**决定**：active project = **pi 启动时 strict binding 状态的 boot-time snapshot**。Session 中通过 bash `cd` 不改变 active project。
...
**Edge case**：pi 启动时 binding 状态不是 `bound`（manifest missing / registry missing / path unconfirmed / conflict）：active project = **none**。这个状态下：
- `$VAULT_<key>` 不查项目 vault，只能使用全局 vault fallback（或按实现拒绝 project lookup）；`$PVAULT_*` 拒绝
- `/secret <key>` 不加 `--global` flag 拒绝写入（项目级无目标）
- sediment 不写 `projects/<任何>/`，Lane A 与 Lane C 都拒绝写入（仅 Lane G 可写全局区域）
- `/memory migrate` 拒绝，并提示 `/abrain bind`
...
项目级 vault (`~/.abrain/projects/<id>/vault/`) 由 `resolveActiveProject(cwd, { abrainHome })` 在 `activate()` 中按 ADR 0017 strict binding 做 snapshot；session 中普通 `cd` 不变更 vault 可见性。`/abrain status` 只展示当前 cwd 的 disk binding 状态，不刷新当前进程 project vault scope；只有显式 `/abrain bind` 会在写入/确认 binding 后刷新当前进程的 binding snapshot。

`/secret set/list/forget` 当前 scope 解析（legacy P1 + B4.5 strict binding 已 ship）：
- 无 flag → boot-time active project（strict binding 状态必须为 `bound`；manifest/registry/path 任一缺失都 fail-closed）
- `--global` → 显式 global opt-out
- `--project=<id>` → 只能等于当前 strict-bound active project；不能绕过本机 local-map 授权去写其他 project
...
| local-map 更新（`/abrain bind`） | atomic read-modify-write：读 `~/.abrain/.state/projects/local-map.json` → merge 当前 path → tmp+rename。B4.5 不在 pi 启动时自动写，不弹阻塞式 prompt；path_unconfirmed 由用户显式 `/abrain bind` 确认。 |
