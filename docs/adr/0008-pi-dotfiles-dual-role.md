...
- **状态**: 部分被 ADR 0012 取代。**`~/.pi` 双重身份的门牌付不变**（pi-stack 开发环境 + 其他项目的 pi 基础环境共存）。但“**以 source 路由区分两者**”的机制在 v6.8 被废弃：`.gbrain-source` dotfile 已删除，gbrain v0.27 multi-source 写入未实现，所有世界级内容都写 gbrain default，项目级内容写 `<projectRoot>/.pensieve/`。“现在 cwd 是哪个身份”仅通过两个 target 的独立检测体现（pensieve：`.pensieve/` 在不在 · gbrain：`gbrain doctor` 能不能走通）。
- **日期**: 2026-05-05
- **决策者**: alfadb
- **依赖**: ADR 0001（pi-stack 定位）/ ADR 0002（superseded by 0012）/ ADR 0004（superseded）/ ADR 0007（offline）
...
| 身份 1 | 身份 2 |
|---|---|
| **pi-stack 的开发环境** | **其他项目的 pi 基础环境** |
| alfadb cd 到 ~/.pi/agent/skills/pi-stack/ 改 pi-stack 代码 | alfadb cd 到 ~/work/some-project 跑 pi |
| 这是 pi-stack 项目本身的工作 | pi 加载 ~/.pi/agent/settings.json 作为全局配置 |

两个身份共用同一个文件树，但**沉淀归属不同**。

更复杂的是 alfadb 跨设备开发：同一个 pi-stack 在设备 A 是 `~/.pi`，设备 B 是 `/data/alfadb/.pi`，设备 C 是 `~/dotfiles/pi`——路径不同，但都应解析到同一 `pi-stack` source。
...
| 4 | `local_path` 注册（最长前缀） | 本机便利（每台机首次 `gbrain sources add pi-stack --path <本机绝对路径>`） |
| 5 | brain default | 用户喜好 |
| 6 | seeded `default` source | 兜底 fallback |

pi-stack **不**重复实现这套路由，**不**自己根据 cwd 推断 source 注入 env。

v6.5.1 补充：gbrain resolver 的结果对交互式 CLI 是权威；但对 background sediment write 还必须经过**写入时 source trust guard**。原因是 `.gbrain-source` 是普通仓库文件，第三方恶意仓也能伪造 `pi-stack` source id。sediment 不重写 resolver，但必须在写入前验证 resolver 结果是否来自可信路径。
...
```
~/.pi/                                             ← 主 dotfiles 仓
├── .gbrain-source                                 ← commit！内容: pi-stack
└── agent/skills/pi-stack/                         ← submodule
    └── .gbrain-source                             ← commit！内容: pi-stack
```

为什么两份：
- `~/.pi/.gbrain-source` —— alfadb cd 到 ~/.pi 任何位置（不在 submodule 内）时，dotfile walk-up 命中根
- `pi-stack/.gbrain-source` —— alfadb 直接 clone pi-stack 到非 ~/.pi 路径独立调试时（如 `~/code/pi-stack`），dotfile walk-up 仍能命中

两份内容**完全相同**（`pi-stack`），不是路径，不会冲突。
...
设备 A (~/.pi)               设备 B (/data/alfadb/.pi)        设备 C (~/dotfiles/pi)
       ...
       │
       ▼
  优先级 3 命中 → 三台设备都解析为 source: pi-stack
       │
       ▼
  本机首次（可选，本机便利）：
    gbrain sources add pi-stack --path <本机绝对路径> --no-federated
...
若 sediment 盲信 resolver，alfadb 只要 `cd` 进该 repo，就可能把恶意仓的项目事件写入真正的 `pi-stack` source。因此 source-router 在写入前执行 trust guard：
...
|---|---|---|
| `~/.pi/` 任何位置（root + submodule + 子目录） | 优先级 3 命中 dotfile（内容 `pi-stack`） | `pi-stack` |
...
|---|---|---|---|
| 项目 source（pi-stack 等） | ✅ 写项目 | ✅ 写 default¹ | ✅ 拆双写¹ |
| `default`（未注册仓 fallback） | ⛔ **拒写**，pending queue | ✅ 写 default¹ | ⛔ **整条 pending**（不允许半写） |

¹ default 写入门槛抬高：confidence ≥ 7 + 3/3 全票同意 scope=cross-project。详见 ADR 0004 § 3.4。
...
`~/.pi/.pensieve/` 现有 6 maxim + 23 decision + 62 short-term + 4 pipeline，**全部**关于 ~/.pi 工作流本身，归 `pi-stack` source（federated=false）。迁移路径详见 ADR 0006 与 migration/steps.md P1。

## 配置位置

`defaults/pi-stack.defaults.json`（package-local fallback / 文档示例，不被 pi 自动加载）：

```json
{
  "piStack": {
    "memory": {
      "projectSource": "pi-stack",
...
- ~/.pi/.pensieve/ 历史沉淀 100% 归 pi-stack source（语义保真）

### 负面
- alfadb 在新机器首次安装：clone ~/.pi 后还要 `gbrain sources add pi-stack --path <本机路径>`（虽然 dotfile 已能让 resolver 命中，但 `local_path` 注册仍是本机便利所需，例如 `gbrain export --dir <auto>`）
