...
- **状态**: 部分被 ADR 0012 取代。**`~/.pi` 双重身份的门牌付不变**（pi-astack 开发环境 + 其他项目的 pi 基础环境共存）。但“**以 source 路由区分两者**”的机制在 v6.8 被废弃：`.gbrain-source` dotfile 已删除，gbrain v0.27 multi-source 写入未实现，所有世界级内容都写 gbrain default，项目级内容写 `<projectRoot>/.pensieve/`。“现在 cwd 是哪个身份”仅通过两个 target 的独立检测体现（pensieve：`.pensieve/` 在不在 · gbrain：`gbrain doctor` 能不能走通）。
- **日期**: 2026-05-05
- **决策者**: alfadb
...
|---|---|---|---|
| 项目 source（pi-astack 等） | ✅ 写项目 | ✅ 写 default¹ | ✅ 拆双写¹ |
