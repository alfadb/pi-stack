...
- 项目本质是 **alfadb 整个记忆/工作流基础设施的重构**，不是简单的代码合并。pensieve 项目作为 pi-stack 组件不存在；记忆基础设施由 gbrain 唯一承载（详见 ADR 0002）。
...
4. **defaults/pi-stack.defaults.json**（package-local fallback / 文档示例；运行时配置走官方 pi settings chain）

5. **gbrain 部署是 alfadb 自决**（pi-stack 不代管）；offlineuff08详见 ADR 0007）只要求 alfadb 提供可用的 gbrain 连接

### "使用即开发"工作流（关键约束）

alfadb 是 **pi-stack 的唯一作者，也是唯一使用者**：
- pi-stack **必须**以 git submodule 形式挂在 `~/.pi/agent/skills/pi-stack/`，可随时 `cd` 进入直接编辑
- pi 加载方式是 `settings.json: packages: ["~/.pi/agent/skills/pi-stack"]`（**local path**，不是 `git:` URL）
- `pi install git:github.com/alfadb/pi-stack` 仅作为"假想他人安装"的备用入口
...
要点：
- 记忆基础设施 = gbrain（postgres + pgvector）唯一承载；gbrain 部署由 alfadb 自决，pi-stack 不提供安装兜底
- 项目记忆 → gbrain `source: pi-stack`（federated=false）
...
- ~/.pi 同时是 pi-stack 开发环境与其他项目的 pi 基础环境，source 路由按 ADR 0008 区分
...
| 实体 | 处置 |
|---|---|
| `alfadb/pi-gstack` repo | archive，整体并入 `pi-stack/skills/` + `pi-stack/extensions/` |
| `alfadb/pi-multi-agent` repo | archive（subtree merge 入 `pi-stack/extensions/multi-agent/`） |
| `alfadb/pi-sediment` repo | archive（subtree merge 入 `pi-stack/extensions/sediment/`） |
| `kingkongshot/Pensieve@feature/auto-sediment-hook` 分支 | 删除（详见 ADR 0005） |
| `kingkongshot/Pensieve@pi` 分支 | 删除（详见 ADR 0005） |
| `~/.pi/agent/extensions/gbrain/` | 移入 `pi-stack/extensions/gbrain/` |
| `~/.pi/agent/extensions/retry-stream-eof.ts` | 移入 `pi-stack/extensions/retry-stream-eof/`（**自有功能，永久 own**） |
| `~/.pi/agent/skills/pi-model-curator/` | 移入 `pi-stack/extensions/model-curator/` |
| `~/.pi/.pensieve/` 数据 | triage + import 到 gbrain `source: pi-stack`，物理目录删除（详见 ADR 0005、ADR 0006） |
...
- ~/.pi 仓需要追踪 pi-stack submodule SHA
...
### 7. 沉淀基础设施
- pi-stack 仓内**不创建** `.pensieve/` 目录（pensieve 项目已退场）
