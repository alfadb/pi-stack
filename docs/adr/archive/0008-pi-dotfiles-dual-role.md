# ADR 0008 - ~/.pi 双重身份与 source 路由（ARCHIVED ORIGINAL）
...
- **日期**: 2026-05-05
- **决策者**: alfadb
- **依赖**: ADR 0001(pi-astack 定位)/ ADR 0002(superseded by memory-architecture.md)/ ADR 0004(superseded)/ ADR 0007(offline)
...
`~/.pi/.pensieve/` 现有 6 maxim + 23 decision + 62 short-term + 4 pipeline,**全部**关于 ~/.pi 工作流本身,归 `pi-astack` source(federated=false)。历史迁移路径详见 ADR 0006 与 [archive/migration/steps-v7-roadmap.md](../../archive/migration/steps-v7-roadmap.md) P1；当前迁移路径见 [migration/abrain-pensieve-migration.md](../../migration/abrain-pensieve-migration.md)。
