...
- **状态**: Accepted。**v6.8（ADR 0012）仍用**— 单 agent + lookup tools 是 sediment 内核，仅运行于 两个独立 target（pensieve / gbrain）之上。
- **日期**: 2026-05-06
- **决策者**: alfadb
...
...任意 markdown 内容...
## Timeline
- **2026-05-06** | pi-stack-sediment — 一句话
```
...
### 模块变更（pi-stack/extensions/sediment/）
...
- `defaults/pi-stack.defaults.json` 加 `singleAgent` block
