...
- **状态**: Accepted。**核心思想保留，tools 已过时**（2026-05-07）— 单 agent + lookup tools 的 sediment 内核被 [memory-architecture.md](../memory-architecture.md) 继承（§8.2 pipeline）。但 lookup tools 从 `gbrain_search/get` 变为 `memory_search/get`，写入从 `gbrain put` 变为 `memory_write/update`。本 ADR 的 markdown 终结符协议（SKIP / SKIP_DUPLICATE / ## GBRAIN）需适配为新的 frontmatter + compiled truth + ## Timeline 格式。
- **日期**: 2026-05-06
- **决策者**: alfadb
