...
| **C — LLM auto-write bg** | sediment agent_end 在显式 miss 后调度 bg promise | project/world/workflow abrain zones（由 curator/writer 路由） | **ADR 0016 + 2026-05-15 sanitizer 修订后**：pre-LLM typed redaction + LLM extractor + ADR 0015 `memory_search` lookup + curator lifecycle ops；safety/storage gates 仅 sanitizer plaintext boundary + schema/lint/exact slug collision/lock/atomic write/audit/git。旧 G2-G13/readiness/rolling/rate/sampling 已删除。 | ✅ 已实现 |
