...
| **C — LLM auto-write bg** | sediment agent_end 在显式 miss 后调度 bg promise | project `.pensieve/` | G2-G13（forceProvisional / disallowMaxim / disallowArchived / maxConfidence / sanitizer ×7 / normalize / triggerPhrase / Trust Boundary prompt / audit raw_text / rolling / extractor model / rate / sample stride / soft near-dup）| ✅ 已实现 |
