...
5. Smoke 覆盖：`smoke-abrain-active-project` 覆盖 strict resolver / bind happy / duplicate path / manifest conflict / path conflict / no-partial-on-path-conflict / concurrent local-map update；`smoke-memory-sediment` 覆盖 unbound target repo refused、cwd/target 错配拒绝与 migrate 主路径；`smoke-abrain-secret-scope` 覆盖 `/secret` project-scope 拒绝与 `/abrain status` read-only；`smoke-abrain-backend-detect` 覆盖 `vault_release(scope='project')` 注册/拒绝路径。
