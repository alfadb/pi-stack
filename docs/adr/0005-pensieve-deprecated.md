# ADR 0005 — pensieve 项目作为 pi-stack 组件退场

- **状态**: **Superseded by ADR 0012**（2026-05-06）— 本 ADR 的前提是 “gbrain 可以接管 pensieve 的项目级职责”，被 gbrain v0.27 multi-source 未实现证伪。
  pensieve target 被复活，sediment 回到“.pensieve/ 项目级 + gbrain default 世界级”双 target 。
- **日期**: 2026-05-05
- **决策者**: alfadb
...
历史上 pi-stack 计划包含 pensieve 项目的 vendor + 端口层（`vendor/pensieve` submodule + `runtime/pensieve/` 完整 own）。v3-v6 演化后发现：

1. **基础设施重复**：pensieve 与 gbrain 都是知识/记忆基础设施，pi-stack 不需要两套
...
v6 双 T0 批判指出：
> alfadb 不是作者也不是 fork owner，对 pensieve 的实际权重主要靠 PR/review 质量和 issue 触觉支撑。这两者都建立在持续 dogfooding 之上。pi-stack 是 alfadb 唯一会高频踩 pensieve 的场所——一旦 pi-stack 完全不用 pensieve，alfadb 对 pensieve 的认知会在 3-6 个月内退化成"读 PR 描述"水平。
...
pensieve `pi` 分支里有部分 alfadb 自己写的内容是 pi-stack 仍想保留的，按以下分类处理：

| 来源 | 处置 |
|---|---|
| 4 个 pipelines (`pipeline.run-when-{committing,planning,reviewing-code,syncing-to-main}.md`) | 提取为 `pi-stack/prompts/{commit,plan,review,sync-to-main}.md`（M10：同时在 gbrain 写一个 page_type=guide 节点，可被反向引用） |
...
- pi-stack 仓体积缩小（无 vendor/pensieve + runtime/pensieve）
- 沉淀去重统一
