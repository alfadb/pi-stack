...
- **状态**: **被 [memory-architecture.md](../memory-architecture.md) 改变前提**（2026-05-07）— 本 ADR 的核心论据 "gbrain = postgres 离线代价大" 在新架构中不再成立：新架构采用纯 markdown+git（决策 2），天然离线可用。ADR 中的两件套兜底（markdown export + read fallback）被 git clone/pull 替代。保留本 ADR 作为离线设计思维的历史档案。
- **日期**: 2026-05-05
- **决策者**: alfadb
...
> **⚠️ 被 memory-architecture.md 改变前提**：以下分析基于 gbrain (postgres+pgvector) 作为唯一存储的假设。新架构采用纯 markdown+git（memory-architecture.md 决策 2），天然离线可用——git clone 即可获得完整知识库。本 ADR 保留作为历史档案，展示从"离线是特殊状态需要兜底"到"离线是默认状态"的思维演进。
