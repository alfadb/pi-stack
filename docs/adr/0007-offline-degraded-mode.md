...
v6 双 T0 批判一致指出：对一般用户 postgres 依赖代价可控；对 **author**（alfadb 自己调试 pi-stack 代码）不可控，因为"记忆系统出问题"恰好是 author 的高频场景。文件型 fallback 在这些场景里是**事实上的最后救命稻草**。

## 决策

### 部署边界（关键）

**pi-stack 不提供 gbrain 安装兜底。也不提供 local pgvector docker-compose。**alfadb 自行负责：
...
pi-stack 只要求"alfadb 提供可用的 gbrain 连接"，不**强制**任何特定的部署形态：
- 不内置 `docker-compose.yml`
- 不预设 postgres 端口、密码、数据卷路径
- 不提供 setup 脚本

理由：
1. alfadb 的 gbrain 部署可能本来就在某个云 postgres 上，pi-stack 强加 local docker 反而打架
2. gbrain 是 alfadb 自己的项目，pi-stack 只是 gbrain 的消费者之一，不应越界管 gbrain 怎么部署
3. brain maxim "single owner" — gbrain 的部署 own 在 gbrain 项目，pi-stack 不重复实现

pi-stack 文档里**说明前置依赖**即可（在 README 安装步骤中提示"先准备好 gbrain 实例"），不**自带**部署。
...
- `gbrain export --source pi-stack --dir ~/.pi/.gbrain-cache/markdown/pi-stack/`
...
4. **gbrain 部署是 alfadb 自决**：pi-stack 只读 `~/.gbrain/config.toml`，不管理其内容
...
| 机制 | 内容 | 跨设备 | 说明 |
|---|---|---|---|
| `.gbrain-source` dotfile | source id 字符串（如 `pi-stack`） | ✅ 可移植 | commit 进 git，多设备 clone 后同步同一 source id |
...
alfadb 可能同一个 pi-stack 在：
- 设备 A：`~/.pi`
- 设备 B：`/data/alfadb/.pi`
- 设备 C：`~/dotfiles/pi`

路径不同，但三台都需要解析到同一个 `pi-stack` source。dotfile 是唯一随 git 同步、内容与路径无关的机制。
...
- `pi-stack/.gbrain-source` —— alfadb 独立 clone pi-stack 到非 ~/.pi 路径（如 `~/code/pi-stack` 临时调试）时，dotfile walk-up 仍能命中

两份内容完全相同（`pi-stack`），不是路径，不会冲突。

### `defaults/pi-stack.defaults.json` 与官方 settings chain
...
`defaults/pi-stack.defaults.json` **不被 pi 自动加载**。它只提供 package-local fallback / 文档示例。运行时配置必须从官方 pi settings chain 读取（`~/.pi/agent/settings.json` + 项目 `.pi/settings.json`，nested object merge）。

该字段**不**用于路由 sediment 写入（路由完全交给 gbrain resolver + ADR 0008 source trust guard）。它有两项辅助用途：
1. **初始化提示**：pi-stack 首次被加载时检查当前仓是否已有 dotfile。没有则提示 alfadb 创建。
...
`~/.pi` 同时是：
- pi-stack 的开发环境（cwd 在 pi-stack 内的子目录）
- 其他项目的 pi 基础环境（用户 cd 到任意项目跑 pi）

source 路由策略（详见 ADR 0008）：
- cwd 落在 `~/.pi/agent/skills/pi-stack/` 下 → dotfile walk-up 解析为 `pi-stack`
- cwd 落在 `~/.pi/` 下但不在 pi-stack/ → dotfile walk-up 解析为 `pi-stack`（dotfiles 仓本身的工作流也归 pi-stack 项目记忆）
...
注：~/.pi/.pensieve/ 现有沉淀关于的就是 ~/.pi 工作流本身，迁移后归 pi-stack source 合理。
