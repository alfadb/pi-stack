...
- **状态**：Accepted（2026-05-09）。**vault 子系统 P0a / P0b / P0c.write / P0c.read + P1 全部已 ship**（2026-05-09 → 2026-05-11）；**B1 workflows lane writer / B3+B7 per-file substrate 剥离 / B4 per-repo 迁移执行器全部已 ship**（2026-05-12，commits `b6a4a49` / `b33f0e4` / `cc40792`→`a819302`→`37f03a6`→`122c0b2`）；**B4.5 Project Binding Strict Mode 已接受、待实施**（[ADR 0017](0017-project-binding-strict-mode.md)：`.abrain-project.json` + `projects/<id>/_project.json` + `.state/projects/local-map.json`；废弃迁移 `--project` 参数）；**sediment 写入锁 stale-lock reclaim + writeAbrainWorkflow TOCTOU re-check 已修**（commits `54835cd` / `4993d77`）。详见下文「实施现状」表与 [migration/abrain-pensieve-migration.md §6](../migration/abrain-pensieve-migration.md#6-adr-0014-状态推进路径)。**待实施**：B4.5 strict binding + P0d（全 TUI onboarding wizard / mask input / `.env` import）+ B5（sediment writeProjectEntry cutover）+ Lane G writer（identity / skills / habits + `/about-me` command）。
...
| **B4: per-repo 迁移执行器** | `/memory migrate` 默认 dry-run（表格预览），`/memory migrate --go` 执行：preflight 检查父仓 + abrain 仓 git clean + `.pensieve` tracked + 有用户条目→逐文件 frontmatter 归一化（kind/scope/schema_version/Timeline）→ pipelines 走 `writeAbrainWorkflow` 路由→ knowledge kinds atomic write 到 `~/.abrain/projects/<id>/<kind-dir>/`→ `git rm` 原件→两边仓各一个 commit（`chore: migrate .pensieve → ~/.abrain/projects/<id>` / `migrate(in): <id> (...)`）。B4.5 后 project id 不再由 `--project` / remote / cwd 推断，而从 strict binding 读取 | commits `cc40792` (初出) + `a819302` (P0：索引重建 + pre-SHA rollback + stale docs) + `37f03a6` (P1/P2：GENERIC_BASENAMES + lane 枚举 + 文档 drift；其中 4 处 name-mismatch 在 `122c0b2` 补完) + `122c0b2` (audit 闭环：gitCommitAll pathspec + audit log 路径 + name-mismatch 补位 + smoke 补示)；`extensions/memory/migrate-go.ts` + `scripts/smoke-memory-sediment.mjs:per-repo migration --go` 覆盖 18 个场景（dirty parent 拒绝 / dirty abrain 拒绝 / happy path 2 knowledge + 2 workflow / legacy frontmatter 归一化 / .index .state 预过滤 / commits / pre-SHA rollback summary / 幂等带保护 + 6 边界）。另外 sediment 写入锁 stale-lock reclaim （commit `54835cd`）4 个 smoke g.1-g.4 不计入本表 |

### 待实施

| 子 phase | 动作 | 验收 | prerequisite |
|---|---|---|---|
| **B4.5: Project Binding Strict Mode** | 新增 `/abrain bind --project=<id>` / `/abrain bind`，写三层身份数据：`<project>/.abrain-project.json`（portable identity）、`~/.abrain/projects/<id>/_project.json`（tracked registry）、`~/.abrain/.state/projects/local-map.json`（local-only multi-path map）。废弃 `~/.abrain/projects/_bindings.md` + git remote / cwd prefix 推断；`/memory migrate` 不再接收 `--project`；未 bound 拒绝 migrate / sediment project writes / project vault scope。详 ADR 0017 | smoke：bind happy / duplicate path / manifest conflict / path conflict / path_unconfirmed / migrate unbound refused / `--project` refused / vault project-scope refused | B5 前置 |
...
| **B5: writer cutover 到 abrain projects 路径** | sediment 三个写入点同步切换：(1) `writeProjectEntry`（`extensions/sediment/writer.ts` 主路径）不再写 `<cwd>/.pensieve/`，改写 `~/.abrain/projects/<id>/<kind-dir>/`；(2) `findProjectEntryFile` walk 起点（`writer.ts:276` 附近的 ­`.pensieve` 硬编码路径，读侧 lookup）同步迁，避免写在新路径但查在旧路径造成 split-brain；(3) `updateProjectEntry` / `mergeProjectEntries` / `archiveProjectEntry` / `supersedeProjectEntry` / `deleteProjectEntry` 等 mutation API 走同一 runtime resolver。整体走 runtime resolver 自动定位，不留 symlink | 迁完某仓后下一次 agent_end 产生的 entry 落 ­~/.abrain/projects/<id>/；14 仓迁完后 grep 整 repo 无残留 hard-coded `.pensieve`。不需 8-phase 过度调控：sediment writer 与 memory facade dual-read 已是长期能力，per-repo 迁完下一次写入自然落新路径 | B4.5 + B4 完成后拍板 |
