...
| **其他 lane writer** | skills / habits 区 writer（Lane G）。**跨项目 knowledge 写路径 ✅ shipped 2026-05-13**：curator/writer 已完整支持 `scope: "world"` 写入 `~/.abrain/knowledge/`，与 B5 同批 cutover。 | 各区有条目产生 | — |

> 七区拓扑说明：extension activate 时 `ensureBrainLayout()`（`brain-layout.ts`）会创建全部七个顶层目录。当前实际使用情况：`vault/`（P0c）+ `projects/`（B5 sediment write + memory Facade dual-read）+ `knowledge/`（curator `scope:"world"` 写路径）+ `workflows/`（B1 lane writer）；identity / skills / habits 是空壳，等待 Lane G writer 落地。只有 `vault/`、`projects/`、`knowledge/`、`workflows/` 有真正的 writer，前三区（identity/skills/habits）完全空壳。
>
