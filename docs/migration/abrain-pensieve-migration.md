...
4. frontmatter normalize（实际是 **full rewrite + selective preserve**，不是「仅补缺失 kind」）：
   ...
   - **`created` / `updated` 三路信号三角测量**（`resolveCreated` / `resolveUpdated`）：老版行为是“created/updated 缺失时填 migrationTimestamp”，会让所有 entry 看起来“刚刚创建”，LLM time-aware rerank 信号丢失。当前版本从三个信号源 triangulate：
     ...
     - `updated = max(git-author-last(%aI), frontmatter.updated, fs.mtime-only-if-untracked)`
     ...
     - 为什么 `updated` 的 `fs.mtime` 仅 untracked 时参与：`git clone` / `checkout` / `git restore` 会把 tracked 文件的 mtime 重置为“拉下来”的时刻，产生虚假的“最近修改”信号；git 是 tracked 文件 modification 的 ground truth。
...
表中条目数是早期粗扫值，包含 legacy seed 与 support file；实际 dry-run 的 `migrateCount` 会排除 seed/support，seed-only 仓可能显示 0 个 project migration item，但 `--go` 仍会 prune seed 并写 migration guard。
...
B4: /memory migrate --go (✅ shipped 2026-05-12)
  ...
  → smoke 覆盖 happy / dirty preflight / legacy frontmatter / .index .state 预过滤 / commits / pre-SHA rollback / 幂等 / collision / mixed batch / post-migration writer-to-abrain 等主路径与审计回归
