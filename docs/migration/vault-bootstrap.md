...
- [x] active project resolver (legacy P1): read-only `resolveActiveProject(cwd)` currently parses `~/.abrain/projects/_bindings.md` via git root → canonical remote → longest cwd prefix and exports `resolveBrainPaths(abrainHome, projectId)` (covered by `npm run smoke:abrain-active-project`). **B4.5 / ADR 0017 将取代此机制**：resolver 改为 strict `.abrain-project.json` + `projects/<id>/_project.json` + `.state/projects/local-map.json`，不再用 remote/cwd 推断。
- [x] project-scoped `/secret`: `set/list/forget` default to the boot-time active project, `--global` is the explicit opt-out, and `--project=<id>` targets a specific project. When no active project resolves, default-scope writes refuse with an actionable reason (`bindings_missing` / `unbound` / `ambiguous_*`). `/secret list` (no flag) prints global plus the active project. `/secret list --all-projects` walks every `~/.abrain/projects/<id>/vault/` (metadata-only via `listAbrainProjects` + `listSecrets`, no decryption). Covered by `npm run smoke:abrain-secret-scope` and `npm run smoke:abrain-active-project`. **B4.5 后 project scope 还必须满足 strict binding 状态 = bound；manifest/registry/path 任一缺失都拒绝 project vault。**
