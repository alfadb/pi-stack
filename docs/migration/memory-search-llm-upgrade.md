...
extensions/memory/
  ...
  index.ts            (改)     memory_search execute 路由：默认走 LLM，env override 走 grep
  search.ts           (保留)   legacy grep+tf-idf fallback / list/get/neighbors
...
[x] Phase 1.2: 改 index.ts execute 路由逻辑（实际实现位置；search.ts 保留 grep fallback）
