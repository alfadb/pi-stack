...
| Phase | 内容 | 工作量 | 阻塞条件 |
|---|---|---|---|
| **Phase 0** | 增强 `_index.md` 格式 | ~30 分钟 | 无 |
| **Phase 1** | LLM-driven search 核心实现 | 2-3 小时 | Phase 0 |
| **Phase 2** | sediment dedupe 接入新 search（ADR 0010 lookup-tools loop） | 1-2 小时 | Phase 1 burn-in 1-2 天 |
| **Phase 3** | 文档对齐 + ADR 状态升级 | 30 分钟 | Phase 1+2 落地 |
...
extensions/memory/
  llm-search.ts       (新建)   核心实现
    - loadIndexForLlm(cwd, settings)
    - selectCandidates(query, indexText, model)
    - rerankFinal(query, candidates, model)
    - llmSearch(params, settings, modelRegistry)
  search.ts           (改)     searchEntries 路由：默认走 LLM，env override 走 grep
  settings.ts         (改)     加 SearchSettings 接口 + 默认值
  types.ts            (改)     SearchSettings 类型
  index.ts            (改)     memory_search 工具描述更新
pi-astack-settings.schema.json (改)     加 memory.search.* 节点
...
Hard rules:
- The query may be a natural-language prompt OR a keyword phrase. Treat
  both as user intent.
...
```bash
MEMORY_SEARCH_GREP_ONLY=1   # 强制 bypass LLM，走原 grep+tf-idf
```

用于 debug、smoke、模型不可用临时降级。
...
- 实测 5 个 query：
  ...
  - 无关 query → 返回空或低分
- LLM 不可用时报 hard error；`MEMORY_SEARCH_GREP_ONLY=1` 后行为退到当前 grep
...
- [ ] [docs/migration/steps.md](./steps.md) Phase 1.3：补 banner 指向本文件，说明 grep+tf-idf 是 fallback 路径
...
- [ ] [ADR 0015](../adr/0015-memory-search-llm-driven-retrieval.md)：状态 Proposed → Accepted
...
```
[ ] Phase 0: 改 extensions/memory/index-file.ts → /memory rebuild --index → 验证 token 量
[ ] Phase 1.1: 写 extensions/memory/llm-search.ts
[ ] Phase 1.2: 改 search.ts 路由逻辑
[ ] Phase 1.3: 改 settings.ts + types.ts + pi-astack-settings.schema.json
[ ] Phase 1.4: 改 index.ts 工具描述
[ ] Phase 1.5: 改 smoke-memory-sediment.mjs 加 LLM 路径 mock
[ ] Phase 1.6: npm run smoke:memory PASS
[ ] Phase 1.7: 实测 5 个 query 验证
[ ] Phase 1.8: commit + push（标 ADR 0015 仍是 Proposed，待 burn-in）
[ ] [burn-in 1-2 天]
