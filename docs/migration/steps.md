...
- **Footer status FSM**（4 态：idle / running / completed / failed）：
  ...
  - 用户感知：`💤 sediment` / `📝 sediment running: auto-write (model=...)` / `✅ sediment completed: N entries` / `⚠️ sediment failed: <reason>`
