...
│   │   ├── migration.ts               # /sediment migrate-one 单文件 legacy migration plan/apply
...
| `extensions/sediment/` | ✅ 部分实现（explicit extractor + LLM dry-run + migrate-one plan/apply；自动 LLM 写入未启用） | Phase 1.4a |
...
| `extensions/sediment/` | alfadb（A 类改造） | ✅ 部分实现（explicit extractor + LLM dry-run + migrate-one plan/apply；自动 LLM 写入未启用） | ✅ |
