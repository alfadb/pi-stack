...
- `sediment/` — ✅ 部分实现：checkpoint/run-window + deterministic explicit `MEMORY:` extractor + `/sediment llm --dry-run`（audit + quality gate + `/sediment llm-report`）+ project-only writer substrate（validate/sanitize/dedupe/lint/lock/atomic write/audit/git best-effort）；自动 LLM 写入仍计划中
