...
- `sediment/` — ✅ Phase 1.4 + ADR 0016：checkpoint/run-window（per-session + RMW 锁）+ deterministic explicit `MEMORY:` extractor（fence-aware）+ `agent_end` 上的 LLM auto-write lane；已删除 dry-run/readiness/rate/sampling/rolling/G2-G13 机械门控，直接调用 LLM extractor + ADR 0015 `memory_search` 找近邻，再由 curator LLM 输出 create/update/delete/skip；project-only writer substrate 只保留 sensitive-info sanitizer（JWT/PEM/AWS/connection URL 等）+ storage integrity（schema/lint/slug collision/lock/atomic write/audit/git best-effort）+ `updateProjectEntry` / `deleteProjectEntry` 更新/删除基座 + `/sediment migration-backups` + `/sediment migrate-one --plan/--apply --yes/--restore --yes` 单文件迁移与恢复。操作手册见 [docs/migration/apply-checklist.md](./docs/migration/apply-checklist.md)
