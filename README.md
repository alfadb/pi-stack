...
- `model-fallback/` — 非对称多模型 fallback：初始模型走 pi 内建指数退避重试（默认 alfadb 配置：claude-code parity，1+9=10 次尝试，1s/2s/4s/…/256s），耗尽后按 `modelFallback.fallbackModels` 切下一个；fallback 模型上任何错误都**不**重试、直接切下一个；列表全失败才停。`initialRetries` 需与 pi `settings.json#retry.maxRetries` 同步。（旧名 retry-stream-eof → retry-all-errors；**自有功能，不向上游 PR**）
