/**
 * pi-sediment agent loop — multi-turn LLM with tool use.
 *
 ...
 * Bounded only by:
 *   - per-call timeout via AbortController (the writer's existing budget)
 *   - tool dispatch failures surfaced as tool errors (model can recover)
 *
 * The model's final assistant TEXT (concatenated text content of the last
 * assistant message with stopReason=="stop") is returned to the caller for
 * protocol parsing (SKIP / ## PENSIEVE / ## GBRAIN).
 */
...
import type { ToolHandler } from "./lookup-tools.js";
...
export interface AgentLoopArgs {
  ...
  /**
   * Per-turn observer for logging. Called once per LLM call and once per
   * tool dispatch. Receives a short kind tag and a free-form detail string.
   */
...
export interface AgentLoopResult {
  /** Final concatenated assistant text content (only when stopReason=="stop"). */
  finalText: string;
  /** Total LLM turns executed. */
  turns: number;
  /** Total tool calls executed across all turns. */
  toolCalls: number;
  /** Last assistant message's stopReason for caller diagnostics. */
  stopReason: string;
  /** Whether the loop ran to completion (stopReason=="stop") or was aborted. */
  ok: boolean;
  /** Last error message if any (signal abort, provider error, etc.). */
  errorMessage?: string;
}
...
export async function runAgentLoop(args: AgentLoopArgs): Promise<AgentLoopResult> {
  ...
  while (true) {
    ...
    // Append the assistant message to the conversation so subsequent turns
    // can reference its tool calls.
    ...
    if (assistant.stopReason === "aborted") {
      return {
        ...
        stopReason: "aborted",
        ok: false,
        errorMessage: assistant.errorMessage ?? "aborted",
      };
    }
    if (assistant.stopReason === "length") {
      // Treat as terminal; caller may still get partial content.
      return {
        finalText: extractText(assistant),
        turns: turn,
        toolCalls,
    ...
    // No tool calls + stopReason==stop → terminal.
    if (calls.length === 0) {
      return {
        finalText: extractText(assistant),
        turns: turn,
    ...
    // Dispatch every tool call from this turn (model may emit multiple in
    // parallel). Append a toolResult message per call. Order matches the
    ...
    // Loop continues; next iteration calls completeSimple with the appended
    // tool results so the model can synthesize the next step.
  }
}
